...
dash.register_page(__name__, path='/B', name="Vendor Performance")

####################### LOAD DATASET #############################
file_path = 'Group 122 Master Data_back up.xlsx'

####################### PAGE LAYOUT #############################
# C1 Chart Preparation
sheet_name = 'C1'
df_c1 = pd.read_excel(file_path, sheet_name=sheet_name)
...
#content
head=  html.Header('Centralized Integrated Vendor Database', style={'textAlign': 'center', 'fontSize': '24px', 'color': 'white', 'backgroundColor': 'black', 'padding': '10px'})
...
layout = dbc.Container([
    ...
    html.Div([
        dcc.Graph(id='c1-chart', figure={
            'data': [
                go.Bar(x=certificate_counts.index, y=certificate_counts.values, marker_color='skyblue')
            ],
            'layout': go.Layout(
